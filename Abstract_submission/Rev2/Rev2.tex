...
% Abstract
 \abstract{In this research, a continuum sensitivity formulation for the shape sensitivity analysis of weakly coupled fluid-structural systems is derived, where the flow is modeled using Navier-Stokes equations and the solid domain is represented as linear elastic domain. The conventional approach to this problem is based on body-conformal grid to represent the solid boundaries. However, these methods will fail for complex shape and/or large deformations of the solid region. This issue can be well addressed by using the immersed boundary method, where additional force terms are added to the governing equations to represent the solid boundaries. Using this approach significant computational saving is achieved by two means: 1) Highly efficient solvers can be utilized on Cartesian grids, and 2) Mesh deformation is not necessary. The sensitivity analysis consists of differentiating the continuum form of the governing equations. In this approach the sensitivity and governing equations use the same differential operators, therefore same solver is utilized for the both systems of equations. This methodology is applied to solve flow over a cylinder and a simplified aeroelastic wing model. The sensitivity of flow field around and on the cylinder, lift of the airfoil, and structural response of the wing are calculated to shape parameter. High-fidelity computational fluid dynamic (CFD) is used to solve the flow and the wing structure is represented with linear finite element model. The sensitivity results are verified using the complex step method where it shows a good agreement.}
